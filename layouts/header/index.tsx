...
function ResponsiveAppBar ()
{
  ...
  const settings = [
    ...
    { name: isLoggedIn ? 'Logout' : 'Login', link: isLoggedIn ? '' : '/auth/login' },
  ];
              ...
              >
                { pages.map( ( page ) => (
                  <MenuItem key={ page } onClick={ handleCloseNavMenu }>
                    <Link
                      href={
                        page === 'Home' ? '/' : page === 'Products' ? '/cms/list' : page === 'About Us' ? '/aboutUs' : '/contactUs' }
                      passHref
                    >
                      <Typography sx={ { textAlign: 'center', color: '#8B0000' } }>{ page }</Typography>
                    </Link>
                  </MenuItem>
                ) ) }
              </Menu>
            ...
            <Box sx={ { flexGrow: 0 } }>
              <Tooltip title="Open settings">
                <IconButton onClick={ handleOpenUserMenu } sx={ { p: 0 } }>
                  <Avatar alt="Remy Sharp" src="/static/images/avatar/2.jpg" sx={ { border: '2px solid #FFD700' } } />
                </IconButton>
              </Tooltip>
              <Menu
                sx={ { mt: '45px' } }
              ...
              >
                { settings.map( ( setting ) => (
                  ...
                  >
                    <Typography sx={ { textAlign: 'center', color: '#8B0000' } }>{ setting.name }</Typography>
                  </MenuItem>
                ) ) }
              </Menu>
            </Box>
